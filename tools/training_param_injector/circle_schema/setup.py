...
ONE_REPO = root_dir.stdout.decode('utf-8')[:-1]
...
class FlatcCaller:
  ...
  def __generate_python_schema(self, schema_path, out_dir ='.'):
    ...
    cmd = ['./' + FLATC_EXE, '--python', '--gen-onefile', '--gen-object-api', '-o', out_dir, schema_path]
    try:
...
setup(
  name='circle_schema',
  version='0.1.0',
  py_modules=['circle_schema_generated', 'circle_traininfo_generated'],
)
